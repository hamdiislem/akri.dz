from django.urls import path
from . import views

urlpatterns = [
    path('', views.home, name='home'),
    path('login/', views.login_view, name='login'),
    path('logout/', views.logout_view, name='logout'),
    path('register/client/', views.register_client, name='register_client'),
    path('register/agency/', views.register_agency, name='register_agency'),
    path('cars/', views.cars_list, name='cars'),
    path('cars/ajouter/', views.add_car, name='add_car'),
    path('cars/<int:car_id>/', views.car_detail, name='car_detail'),
    path('cars/<int:car_id>/modifier/', views.modifier_voiture, name='modifier_voiture'),
    path('cars/<int:car_id>/supprimer/', views.supprimer_voiture, name='supprimer_voiture'),
    path('cars/<int:car_id>/disponibilite/', views.toggle_disponibilite, name='toggle_disponibilite'),
    path('profile/client/', views.profile_client, name='profile_client'),
    path('profile/client/edit/', views.edit_profile_client, name='edit_profile_client'),
    path('profile/agency/', views.profile_agency, name='profile_agency'),
    path('profile/agency/edit/', views.edit_profile_agency, name='edit_profile_agency'),
    path('profile/admin/', views.profile_admin, name='profile_admin'),
    path('account/delete/', views.delete_account, name='delete_account'),
    path('dashboard/client/', views.dashboard_client, name='dashboard_client'),
    path('dashboard/agency/', views.dashboard_agency, name='dashboard_agency'),
    path('dashboard/admin/', views.dashboard_admin, name='dashboard_admin'),
    path('bookings/<int:booking_id>/confirmer/', views.confirmer_booking, name='confirmer_booking'),
    path('bookings/<int:booking_id>/annuler/', views.annuler_booking, name='annuler_booking'),
    path('bookings/<int:booking_id>/completer/', views.completer_booking, name='completer_booking'),
    path('admin/agences/<int:agency_id>/verifier/', views.admin_verifier_agence, name='admin_verifier_agence'),
    path('admin/agences/<int:agency_id>/bannir/', views.admin_bannir_agence, name='admin_bannir_agence'),
    path('admin/agences/<int:agency_id>/debannir/', views.admin_debannir_agence, name='admin_debannir_agence'),
    path('admin/clients/<int:client_id>/bannir/', views.admin_bannir_client, name='admin_bannir_client'),
    path('admin/clients/<int:client_id>/debannir/', views.admin_debannir_client, name='admin_debannir_client'),
    path('admin/clients/<int:client_id>/supprimer/', views.admin_supprimer_client, name='admin_supprimer_client'),
    path('admin/agences/<int:agency_id>/supprimer/', views.admin_supprimer_agence, name='admin_supprimer_agence'),
    path('bookings/<int:booking_id>/avis/', views.submit_review, name='submit_review'),
    path('bookings/<int:booking_id>/evaluer-client/', views.agency_evaluer_client, name='agency_evaluer_client'),
    path('clients/<int:client_id>/profil/', views.agency_view_client_profile, name='agency_view_client_profile'),
    path('admin/clients/<int:client_id>/profil/', views.admin_view_client, name='admin_view_client'),
    path('admin/agences/<int:agency_id>/profil/', views.admin_view_agency, name='admin_view_agency'),
    path('admin/bookings/<int:booking_id>/annuler/', views.admin_cancel_booking, name='admin_cancel_booking'),
    path('tickets/', views.my_tickets, name='my_tickets'),
    path('tickets/nouveau/', views.create_ticket, name='create_ticket'),
    path('admin/tickets/', views.admin_tickets, name='admin_tickets'),
    path('admin/tickets/<int:ticket_id>/repondre/', views.admin_respond_ticket, name='admin_respond_ticket'),
]
